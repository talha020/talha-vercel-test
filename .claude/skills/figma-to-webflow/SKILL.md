...
1. **Variables.** `data_variable_tool > create_variable_collection` named `Colors`, then one `create_color_variable` per token on the build sheet, using the Figma semantic names when the file has them. Then a second collection `Responsive` with modes `Tablet` (`breakpoint_id: medium`), `Mobile Landscape` (`small`) and `Mobile Portrait` (`tiny`), holding `Heading Size/H1..H6`, `Text Size/XLarge..Small`, `Padding Global`, `Padding Section/Large|Medium|Small|Tiny` and `Container/Large` as size variables in em, with a value per mode (`update_size_variable` with `mode_id`). The values and the reasoning are in `references/conventions.md` section 1b. Type and spacing classes then reference these variables and need no breakpoint CSS of their own.
...
3. **Tag resets.** On a fresh site the `body`, `h1`–`h6`, `p` and `a` tag styles do not exist yet and `update_style` on them fails, so put the resets in the same head block: `body { font-family; color; background-color; line-height }`, `h1, h2, h3, h4, h5, h6, p { margin: 0; font-weight: inherit }`, `a { color: inherit; text-decoration: none }`. Reviewers see default blue links in Safari otherwise.
...
5. Verify the section: read back its tree and the new classes (`query_elements`, `query_styles`) and, when a Designer session is open, `element_snapshot_tool`; otherwise publish to staging and compare the live page with the Figma screenshot. Fix spacing and type before moving on; the reviewer measures both.
